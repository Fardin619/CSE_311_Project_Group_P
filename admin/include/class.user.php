<?php
    include "db_config.php";
        class user
        {
            ...
            {
                $this->db=new mysqli(DB_SERVER,DB_USERNAME,DB_PASSWORD,'barely_alive');
            ...
            public function add_room($roomname, $room_qnty, $available, $booked,$difficulty,$price)
            {

                    $available=$room_qnty;
                    $booked=0;

                    $sql="INSERT INTO room_category SET roomname='$roomname', available='$available', booked='$booked', room_qnty='$room_qnty', facility='$facility', price='$price'";
   ...
   FROM rooms WHERE (checkin <= '$checkin' AND checkout >='$checkout') OR (checkin >= '$checkin' AND checkin <='$checkout') OR (checkin <= '$checkin' AND time >='$slot') )";
